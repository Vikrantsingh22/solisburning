...
import { Area, CartesianGrid, Label, XAxis } from "recharts";
  ...
  },
  {
    ...
    description: {
      sections: [
        {
          title: "Quick Summary",
          content: "Condom presale raised over $900k and performed exit scam.",
        },
        {
          title: "Details of the Exploit",
          content:
...
export function ExploitsTable() {
  ...
  const filteredData = exploitsData
    .filter((item) => {
      ...
      const searchTermMatch =
        ...
        (item.description.sections[0].content &&
          item.description.sections[0].content
            .toLowerCase()
            .includes(searchTerm.toLowerCase()));
      ...
      // Type filter
      const typeMatch =
        filterType === null || item.scam_type.type === filterType;

      // Status filter
      const statusMatch =
        filterStatus === null || item.audit_status === filterStatus;
    ...
    .sort((a, b) => {
      if (sortBy.column === "funds") {
        return sortBy.direction === "asc"
          ? a.funds_lost - b.funds_lost
          : b.funds_lost - a.funds_lost;
      }
  ...
  // Get brief description from first section
  const getDescription = (item) => {
    return item.description.sections[0].content.substring(0, 100) + "...";
  };

  // Function to handle clicking on a project
  const handleProjectClick = (exploit) => {
    // Format exploit data to match TableCellViewer props structure
    const cellViewerItem = {
      header: exploit.project_name,
      type: exploit.scam_type.type,
      status: exploit.audit_status,
      target: formatFunds(exploit.funds_lost),
      limit: exploit.date.display,
      category: exploit.name_categories,
      funds_returned: formatFunds(exploit.funds_returned),
      proof: exploit.proof_link,
      twitter_link: exploit.twitter_link,
      website_link: exploit.website_link,
      token_address: exploit.token_address,
      telegram: exploit.telegram_link,
      reviewer: "Not Assigned", // Default value, adjust as needed
      description: exploit.description.sections,
    };

    setSelectedExploit(cellViewerItem);
  };
    ...
    <Card>
      <div className="flex flex-col md:flex-row items-center p-4 gap-4">
        <div className="relative flex-1 min-w-0">
          ...
          <Input
            placeholder="Search project name, type, or description"
            className="pl-10"
            value={searchTerm}
        ...
        <Table>
              ...
              <TableHead className="w-[180px]">
                ...
                >
                  ...
                  {sortBy.column === "project" && (
                    <ChevronDownIcon
                      className={`h-4 w-4 ${
                        sortBy.direction === "asc" ? "rotate-180 transform" : ""
                      }`}
                    />
                  )}
                </button>
          ...
          <TableBody>
            {filteredData.length === 0 ? (
              ...
              filteredData.map((exploit, index) => (
                <TableRow key={index}>
                  <TableCell className="font-medium">
                    {/* Wrap the project name with our custom integration */}
                    <Drawer>
                  ...
                  <TableCell>{exploit.scam_type.type}</TableCell>
                  <TableCell>{exploit.date.display}</TableCell>
                  <TableCell>{formatFunds(exploit.funds_lost)}</TableCell>
                  <TableCell>
                    <Badge
                      variant={
                        exploit.audit_status === "Audited"
                          ? "outline"
                          : "secondary"
                      }
                    >
...
// Modified TableCellViewer to work within the Drawer content
function TableCellViewer({ item }) {
  const useIsMobile = () => {
    // Simple hook to determine if mobile view
    // You might want to replace with your actual implementation
  ...
  return (
    <DrawerContent>
      <DrawerHeader className="gap-1">
        ...
        <DrawerDescription>Category: {item.category}</DrawerDescription>
      </DrawerHeader>
      <div className="flex flex-col gap-4 overflow-y-auto px-4 text-sm">
        ...
        <form className="flex flex-col gap-4">
          {/* <div className="flex flex-col gap-3">
            <Label htmlFor="header">Project</Label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-3">
              ...
              {/* <Select defaultValue={item.type}>
                ...
                <SelectContent>
                  <SelectItem value="Flash Loan Attack">
                    Flash Loan Attack
                  </SelectItem>
                  <SelectItem value="Reentrancy">Reentrancy</SelectItem>
                  <SelectItem value="Smart Contract Exploit">
                    Smart Contract Exploit
                  </SelectItem>
                  <SelectItem value="Oracle Manipulation">
                    Oracle Manipulation
                  </SelectItem>
                  <SelectItem value="Private Key Compromise">
            ...
            <div className="flex flex-col gap-3">
              ...
              {/* <Select defaultValue={item.status}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="Partial Audit">Partial Audit</SelectItem>
                </SelectContent>
              </Select> */}
              Fund Returned:
              <div>{item.funds_returned}</div>
            </div>
          </div>
          {/* <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-3">
              <Label htmlFor="target">Funds Lost</Label>
          ...
          {item?.token_address?.length > 1 ? (
            <div className="flex flex-col gap-3">
              ...
              {/* <Select defaultValue={item.reviewer || "Not Assigned"}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="External Auditor">
                    External Auditor
                  </SelectItem>
                  <SelectItem value="Community Review">
                    Community Review
                  </SelectItem>
                </SelectContent>
              {/* <Select defaultValue={item.reviewer || "Not Assigned"}>
          ...
          {item?.twitter_link?.length > 1 ||
          item?.telegram?.length > 1 ||
          item?.website?.lenght > 1 ? (
            <div className="grid grid-cols-2 gap-4">
              {item?.twitter_link?.length > 1 ? (
                <div className="flex flex-col gap-3">
                  {/* <Label htmlFor="target">Funds Lost</Label>
